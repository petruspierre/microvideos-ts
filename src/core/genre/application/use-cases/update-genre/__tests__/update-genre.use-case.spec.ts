...
describe('UpdateGenreUseCase Unit Tests', () => {
  ...
  describe('execute method', () => {
    it('should throw an entity validation error when categories id not exists', async () => {
      expect.assertions(3);
      const genre = Genre.fake().one().build();
      ...
      try {
        ...
        expect(e.error).toStrictEqual([
          {
            categories_id: [
              'Category with id(s) 4f7e1c30-3f7a-4f51-9f4a-3e9c4c8f1a1a not found',
              'Category with id(s) 7f7e1c30-3f7a-4f51-9f4a-3e9c4c8f1a1a not found',
            ],
          },
        ]);
      }
